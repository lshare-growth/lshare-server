...
class StudyCreateIntegrationTest extends IntegrationTestContainer {
    ...
    class StudyCreateTest {
        ...
        void study_create_success_test() {
            // given
        ...
        @Test
        @DisplayName("존재하지 않는 회원이 스터디를 개설하려고 시도하면 MemberNotFoundException이 발생한다.")
        void study_create_failure_test() {
            // given
            ...
            // when, then
            assertThrows(MemberTypeException.class, () -> studyCommandFacade.createStudy(memberId, newStudy, tagNames));
        }
    }
}
